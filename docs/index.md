...
書籍 [「JavaScriptレスの動的UI開発 htmx入門」太田智暉 著、C＆R研究所](https://www.c-r.com/book/detail/1595) （以下で "htmx本" と略する）のサンプルコードを読んで [htmx](https://htmx.org/) を活用したwebアプリケーションを開発する手法を学ぼうと思う。htmxそのものについての解説はhtmx本に譲る。webアプリケーションの動作環境をTypeScriptで構築することと、Playwrightでend-to-endテストを実装することについて、わたしなりの工夫をした。どうやったか以下に記述する。
...
console.log("xxx")でメッセージをコンソールに出力するのは簡単だがやり過ぎるとコンソールの表示がごちゃごちゃになり見づらくなる。ログを出力先をファイルに変更したい。そこで [LogTape](https://www.npmjs.com/package/logtape) というロギングフレームワークを導入します。
...
`http://localhost:3001/section3` にアクセスして、ボタンをクリックしたらボタンのラベルが切り替わるかどうかを検証するテストコードを書きました。
...
`tests/section3.e2e.ts` を実行するには、`http://localhost:3001` の webアプリを起動しておく必要があります。`bun test` を実行して終了したら、webアプリを停止したい。タイピングが大変です。コマンド操作を簡便にするために `package.json` の `scripts` に `testmain` サブコマンドを追加しました。

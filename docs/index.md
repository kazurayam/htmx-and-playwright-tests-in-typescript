...
- htmx本はテストをどう書くかに触れていない。わたしはまずテストを書いて、次にhtmx本のサンプルコードを写して、ひとつひとつ動作を確認するという手順（いわゆるtest-first）でやりたい。だから [Playwright](https://playwright.dev/docs/intro) でwebアプリをテストしよう。
...
### テストコードを格納する場所として `tests` ディレクトリを追加する
...
### PlaywrightのAPIを使ってブラウザを操作するテストを書く
...
このテストを実行しました。
    ...
    tests/play-on-browser.test.ts:
    browser.browserType().executablePath()=/Users/kazuakiurayama/Library/Caches/ms-playwright/chromium-1228/chrome-mac-arm64/Google Chrome for Testing.app/Contents/MacOS/Google Chrome for Testing

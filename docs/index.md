...
`packages` ディレクトリの下に `my-app` パッケージと `todo-app` パッケージがある。それぞれのWebアプリはどちらもHonoを基盤としてJSXとhtmxを利用したアプリであって独立している。`` todo-app`のWebアプリのコードが`my-app`のWebアプリのコードに依存したりはしていない。ただし詳しくは後で述べるが `todo-app `` のテストコードが `my-app` の `tests` の中にある `BrowserDriverChromium` class を参照するようにしたい。だから `todo-app` が `my-app` を参照できるようにbunの設定を作る必要がある。
...
このプロジェクトには３つの `package.json` ファイルがありますが、その冒頭にある `name` プロパティの値をどうするかが割と大事です。

npm Docの name に関する説明はこう述べています。

> If you plan to publish your package, the most important things in your package.json are the name and version fields as they will be required. The name and version together form an identifier that is assumed to be completely unique. Changes to the package should come along with changes to the version. If you don’t plan to publish your package, the name and version fields are optional.

わたしはGitレポジトリ [htmx-and-playwright-tests-in-typescript](https://github.com/kazurayam/htmx-and-playwright-tests-in-typescript) の成果物をnpmにパブリッシュするつもりはない。だからnmpレポジトリの中でuniqueな名前を与えなければならぬ義理はない。しかし `todo-app` が `my-app` の成果物を参照したいと企んでい。それを可能にするには `todo-app` と `my-app` のパッケージ名を整った名前にしておくことが必須になります。だからnmpにパブリッシュできるようなユークな名前を付けることを常日頃から心がけておく方が得策だ。
...
するとbunがpackage.jsonファイルを読み込み解釈して `node_modules` ディレクトリの中身を適宜書きかえる。

ちなみにわたしが `bun install` コマンドを実行した後でVSCodeの画面で `node_modules` ディレクトリの中身を目視しようとしたが、変化が見られなくて「あれ？」と驚くことがあった。VSCodeをいったん止めて再起動したら `node_modules` の中身が変わっているのが見えた。つまりVSCodeに画面をリフレッシュするチャンスを与えなければならないということのようだ。VSCodeを上手に設定すれば回避できそうだが、それは別の話題。
...
### 6.4 todo-appがmy-appのBrowserDriverChromiumを参照できない問題を対策する
...
を実行した。もう一度VSCodeのエディタに戻ってみた。まだ下波線が表示されていた。ただし警告メッセージが別の物になっていた。
...
どういうことか？ちなみに `packages/my-app/package.json` ファイルの冒頭にこう書いてある.
...
なるほど。TS2306のエラーを解消するには `packages/my-app/index.ts` が `BrowserDriverChromium` を明示的にexportすればいいのだろう。やってみた。
...
VSCodeのエディタで `todo.e2e.ts` を確認した。するとようやく下波線が消えた。

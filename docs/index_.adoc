...
書籍 link:https://www.c-r.com/book/detail/1595[「JavaScriptレスの動的UI開発 htmx入門」太田智暉 著、C＆R研究所] （以下で "htmx本" と略する）のサンプルコードを読んで link:https://htmx.org/[htmx] を活用したwebアプリケーションを開発する手法を学ぼうと思う。htmxそのものについての解説はhtmx本に譲る。webアプリケーションの動作環境をTypeScriptで構築することと、Playwrightでend-to-endテストを実装することについて、わたしなりの工夫をした。どうやったか以下に記述する。
...
console.log("xxx")でメッセージをコンソールに出力するのは簡単だがやり過ぎるとコンソールの表示がごちゃごちゃになり見づらくなる。ログを出力先をファイルに変更したい。そこで link:https://www.npmjs.com/package/logtape[LogTape] というロギングフレームワークを導入します。

...
## PlaywrightのBrowser APIを使ってブラウザを操作するテストを書く

ブラウザで `http://localhost:3001/section3` を開き最上部のボタンを押下すると、ボタンのラベルが "クリック" から "GETリクエスト!" という文字に置き換わりました。逆にいえば「web画面の最上部のボタンを押下すると、ボタンのラベルが "GETリクエスト!" に置き換わるはず」と言えます。webアプリがこの条件を満足しているかどうかを検証するテストをPlaywrightで実装し動かしてみます。
...
### console.log("xxx")の代わりにロギングフレームワークを使う

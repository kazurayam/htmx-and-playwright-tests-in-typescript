...
:REPOS_URL: https://github.com/kazurayam/htmx-and-playwright-tests-in-typescript/tree/develop
...
htmx本のCHAPTER07「サンプルアプリの作成」SECTION-25にChatアプリが紹介されている。著者はPython言語で書いたwebアプリケーションの link:https://github.com/tomo1227/htmx_book_app/blob/main/src/chat.py[chatアプリのPython言語による実装] をGitHubで公開している。それと同等のものをわたしはTypeScript言語で実装した。

=== 7.1 Chatプリ設計の概要

- WebSocketプロトコルで通信するクライアントとサーバを作る。
- サーバーはTypeScript言語で書き、Bunの上で動かす。Bunがサーバー・サイドのWebSocket APIを提供しているのでこれを利用してChatサーバーを実現する。broadcastサービスを実現するのにBunの link:https://bun.com/docs/guides/websocket/pubsub[publish-subscribeパターンの実装] を活用する
...
- Chatアプリの３通りの手法で実装する。素朴な実装手法からより高度な手法へと段階的にコードを書き変える。
  ...
  2. 二番目のChatアプリでは link:https://htmx.org/extensions/ws/[HtmxのWebSocket Exetension] を導入する。サーバとクライアントのコードをかなり大きく書き変える。
...
4. link:https://bun.com/reference/bun/serve[Bun API Reference / serve]
5. link:https://developer.mozilla.org/en-US/docs/Web/API/WebSocket[MDN / WebSocket]
...
- `packages/chat-app/static/htmx/ext/ws.js` --- link:https://www.jsdelivr.com/package/npm/htmx.org[jsdeliverのhtmx-ext-ws] からダウンロードした。
- `packages/chat-app/static/htmx/htmx.min.js` --- link:https://www.jsdelivr.com/package/npm/htmx.org[jsdeliverのhtmx..org] からダウンロードした。
- `packages/chat-app/static/styles/chat.css` --- htmx本の著者が公開している link:https://github.com/tomo1227/htmx_book_app/blob/main/static/styles/chat.css[GitHubレポジトリ] からダウンロードした
...
image:{PAGES_URL}/images/071-chatpage-onload.png[]

メッセージの入力フィールドに「こんにちは」と入力して送信ボタンを押すと、画面の下の方に「from you: こんにちは」と表示される。

image:{PAGES_URL}/images/072-chat-echo.png[]
...
image:{PAGES_URL}/images/073-chat-broadcast.png[]
...
image:{PAGES_URL}/images/074-chat-server-side-event.png[]

==== 7.4.4 chat-app/src/vanilla-javascriptのソース

- link:{REPOS_URL}/packages/chat-app/src/vanilla-javascript/index.ts[chat-app/src/vanilla-javascript/index.ts]

[source,typescript]
----
include::../packages/chat-app/src/vanilla-javascript/index.ts[]
----

- link:{REPOS_URL}/packages/chat-app/src/vanilla-javascript/index.html[chat-app/src/vanilla-javascript/index.html]

[source,html]
----
include::../packages/chat-app/src/vanilla-javascript/index.html[]
----

- link:{REPOS_URL}/packages/chat-app/src/vanilla-javascript/broadcast.ts[chat-app/src/vanilla-javascript/broadcast.ts]

[source,typescript]
----
include::../packages/chat-app/src/vanilla-javascript/broadcast.ts[]
----
...
- link:{REPOS_URL}/packages/chat-app/src/htmx-ws/index.ts[chat-app/src/htmx-ws/index.ts]

[source,typescript]
----
include::../packages/chat-app/src/htmx-ws/index.ts[]
----

- link:{REPOS_URL}/packages/chat-app/src/htmx-ws/index.html[chat-app/src/htmx-ws/index.html]

[source,html]
----
include::../packages/chat-app/src/htmx-ws/index.html[]
----

- link:{REPOS_URL}/packages/chat-app/src/htmx-ws/broadcast.ts[chat-app/src/htmx-ws/broadcast.ts]

[source,typescript]
----
include::../packages/chat-app/src/htmx-ws/broadcast.ts[]
----
...
`src/vanilla-javascript/index.ts` と `src/htmx-ws/index.ts` はどちらも `index.html` ファイルを読み込んでHTML画面を応答している。この方式は単純で分かりやすい。プログラミングの教材としてのChat画面を実現するにはこれで足りる。しかしちょっとでも複雑さを増したHTMLを実現するには不十分だ。menuとnaviとbodyとfooterというふうにHTMLを部品に分割し、コンポーネントを組み合わせることでviewを組み立てる方式を実現したい。そのためには link:https://hono.dev/docs/guides/jsx[JSX] が最適だ。`src/htmx-ws` のコードを土台とし、HonoとJSXを導入しよう。
...
- link:{REPOS_URL}/packages/chat-app/src/htmx-ws/index.tsx[chat-app/src/hono-jsx/index.tsx]

[source,typescript]
----
include::../packages/chat-app/src/hono-jsx/index.tsx[]
----

- link:{REPOS_URL}/packages/chat-app/src/hono-jsx/layout.tsx[chat-app/src/hono-jsx/layout.tsx]

[source,html]
----
include::../packages/chat-app/src/hono-jsx/layout.tsx[]
----

- link:{REPOS_URL}/packages/chat-app/src/hono-jsx/chatform.tsx[chat-app/src/hono-jsx/chatform.tsx]

[source,html]
----
include::../packages/chat-app/src/hono-jsx/chatform.tsx[]
----

- link:{REPOS_URL}/packages/chat-app/src/hono-jsx/broadcast.tsx[chat-app/src/hono-jsx/broadcast.tsx]

[source,typescript]
----
include::../packages/chat-app/src/hono-jsx/broadcast.tsx[]
----
...
1. link:{REPOS_URL}/packages/chat-app/src/vanilla-javascript/index.ts[`chat-app/src/vanilla-javascript/index.ts`]
2. link:{REPOS_URL}/packages/chat-app/src/vanilla-javascript/broadcast.ts[`chat-app/src/vanilla-javascript/broadcast.ts`]
3. link:{REPOS_URL}/packages/chat-app/src/htmx-ws/index.ts[`chat-app/src/htmx-ws/index.ts`]
4. link:{REPOS_URL}/packages/chat-app/src/htmx-ws/broadcast.ts[`chat-app/src/htmx-ws/broadcast.ts`]
5. link:{REPOS_URL}/packages/chat-app/src/hono-jsx/index.tsx[`chat-app/src/hono-jsx/index.tsx`]
6. link:{REPOS_URL}/packages/chat-app/src/hono-jsx/broadcast.tsx[`chat-app/src/hono-jsx/broadcast.tsx`]
...
link:{REPOS_URL}/packages/chat-app/tests[`packages/chat-app/tests`]
...
- link:{REPOS_URL}/packages/chat-app/tests/index.e2e.ts[`packages/chat-app/tests/index.e2e.ts`]

[source,typescript]
----
include::../packages/chat-app/tests/index.e2e.ts[]
----

- link:{REPOS_URL}/packages/chat-app/tests/broadcast.e2e.ts[`packages/chat-app/tests/broadcast.e2e.ts`]

[source,typescript]
----
include::../packages/chat-app/tests/broadcast.e2e.ts[]
----

- link:{REPOS_URL}/packages/chat-app/tests/broadcast-dual.e2e.ts[`packages/chat-app/tests/broadcast-dual.e2e.ts`]

このテストではブラウザを２つ立ち上げている。いっぽうのブラウザのChat画面にメッセージを入力したら、他方のブラウザのChat画面に同じメッセージが表示されることを確認している。

[source,typescript]
----
include::../packages/chat-app/tests/broadcast-dual.e2e.ts[]
----
...
{
  "scripts": {
include::../packages/chat-app/package.json[line=14..20]
----
...
link:{REPOS_URL}/packages/chat-app/bunfig.toml[chat-app/bunfig.toml] に下記のような設定を書いた。

[source,toml]
----
include::../packages/chat-app/bunfig.toml[]
----

これによって環境変数 `SERVER_NAME` のデフォルト値を設定することができた。さらに `chat-app/package.json` に下記のように書いてある。

- link:{REPOS_URL}/packages/chat-app/package.json[chat-app/package.json]
----
include::../packages/chat-app/package.json[lines=14..19]
----

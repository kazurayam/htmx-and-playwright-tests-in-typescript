...
4. htmx本はテストをどう書くかについてまったく触れていない。テスト無しのプログラム開発はありえない。わたしは link:https://playwright.dev/docs/intro[Playwright] を使ってwebアプリをテストしよう。
...
このディレクトリをGitレポジトリのルート・ディレクトリとする。このディレクトリを **$ROOT** という記号で表すことにする。下記のようにしてシェル変数を定義した。
...
将来的にこのレポジトリをいわゆる「モノレポ」にしたいので、サブディレクトリ `packages` を作り、その下にwebアプリケーションを格納することにする。
...
なるほど。 `todo-app.e2e.ts` が `my-app` パッケージTS2306のエラーを解消するには `packages/my-app/index.ts` が `BrowserDriverChromium` を明示的にexportすればいいのだろう。やってみた。

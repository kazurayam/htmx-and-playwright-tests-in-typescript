...
書籍 [「JavaScriptレスの動的UI開発 htmx入門」太田智暉 著、C＆R研究所](https://www.c-r.com/book/detail/1595) （以下で "htmx本" と略する）のサンプルコードと [Qiita記事](https://qiita.com/twrcd1227/items/7bce18167fb02ec22729) をよりどころとつつ、 [htmx](https://htmx.org/) を活用したWebアプリケーションを開発する手法を学ぼうと思う。htmxそのものについてはhtmx本に譲る。webアプリケーションの動作環境をTypeScriptで構築すること、およびPlaywrightでテストを実装することについて、わたしなりの工夫をした。どんな工夫をしたか以下に記述する。自分のための忘備録として。
...
2.  htmxを採用したHTMLをブラウザ上で開いて動きを確認するためにはバックエンドとしてのwebアプリケーションが必要だ。htmx本の著者はPythonで書いたwebアプリケーションの [ソースコード一式](https://github.com/tomo1227/htmx_book_app) をGitHubで公開している。それはさておき、わたしはwebアプリをTypeScriptで書きたい。

3.  webアプリを [bun](https://bun.com/docs) と [Hono](https://hono.dev/) の上で構築したい。Node.jsとExpressではなくて。

4.  htmx本はテストをどう書くかに触れていない。わたしはまずテストを書いて、次にhtmx本のサンプルコードを写して、ひとつひとつ動作を確認するという手順（いわゆるtest-first）でやりたい。だから [Playwright](https://playwright.dev/docs/intro) でwebアプリをテストしよう。

技術的な課題がある。Playwrightの公式ドキュメント [Getting started/Installation](https://playwright.dev/docs/intro) を見れば明らかだが、Playwrightのドキュメントは [Node.js](https://nodejs.org/) 上で実行することを前提している。bunの上でPlaywrightを動かす事例がネット上にほとんど見当たらない。だからわたしは自力で答えを見つけなければならない。
...
ちゃんとしたHTTP応答が表示されました。webサーバーを停止するにはコマンドラインで CTRL+C と入力します。
...
何が起きたのか？要点だけを述べます。htmxに関する詳細な説明が必要なら皆様も [htmx本](https://www.c-r.com/book/detail/1595) を購入して読んでください。 `section3.tsx` のなかには下記のように ht-get属性が書いてある。
...
htmx本にはhtmxのたくさんの機能を紹介するサンプルコードが紹介されています。htmx本を読みながら、上記で `src/section3.jsx` を作ったのと同じようにして \`section4.tsx\`を追加し、\`section5.tsx\`を追加していくことができます。
...
`playwright-chromium` パッケージはChromiumブラウザを含むパッケージでサイズが巨大です。わたしの環境ではダウンロードに4分以上かかった。気長に待て。
...
Playwrightが提供するBrowser APIライブラリを使ってテストがChromiumブラウザを起動し、Webサイト `http://example.com` を開いて、HTMLの中身を検証し、それが済んだらブラウザを開く、というテストを書きました。
...
Playwrightによるテストの書き方を教えるwebサイトは山ほどある。たとえば ["Playwright &gt; Getting Started &gt; Writing tests" By Microsoft](https://playwright.dev/docs/writing-tests)。そのサンプルコードはこう書いている。

    import { test, expect } from '@playwright/test';

`bun:test` からimportした `test` と `expect` と `@playwright/test` からimportした `test` と `expect` とはまったく別物だ。わたしはあえて `bun:test` を選んでテストを書くことにした。

### bun testコマンドでテストを実行する
...
console.log("xxx")でメッセージをコンソールに出力するのは簡単だがやり過ぎるとコンソールの表示がごちゃごちゃになり見づらくなる。ログを出力先をファイルに変更したいしたい。そこで [LogTape](https://www.npmjs.com/package/logtape) というロギングフレームワークを導入します。
...
この段階で `bun test ./tests/play-on-browser-using-logtape.test.ts` を実行してもまだログが出力されません。LogTapeを働かせるには設定を加えることが必要です。

`$PROJECT/logtape.ts` ファイルを書きます。
...
`$PROJECT/bunfig.toml` を修正します。

    include::{MY-APP_DIR}/bunfig.toml

`bunfig.toml` の `[test]` セクションに `preload = [ "./logtape.ts" ]` と記述することにより、`bun test` コマンドが実行された時にテストコードが実行される前に一度だけコマンド `bun ./logtape.ts` が実行されます。`logtape.ts` がLogTapeの初期設定を行い sink や logger が準備されます。`play-on-borowser-using-logtape.test.ts` は
...
テストコードのみならずwebアプリ本体もロギングフレームワークを利用して、詳細なログ情報をファイルに出力することができる。
...
clickに反応してボタンのラベルが切り替わるかどうかをテストしよう。そのようなテストをPlaywrightのAPIを利用して書きたい。テストを `bun test` コマンドで実行したい。どうすればいいのか？これが次のテーマです。

### 5.1 E2Eテストを実行する

`http://localhost:3001/section3` にアクセスして、ボタンをクリックしたらボタンのラベルが切り替わるかどうかを検証するテストコードを `tests/section3.e2e.ts` に書きました。
...
`tests/section3.e2e.ts` を実行するには、`http://localhost:3001` の webアプリを起動しておく必要があります。操作を簡便にするために `package.json` の `scripts` に `testmain` サブコマンドを追加しました。
...
`testmain` サブコマンドを実行すると、まずwebアプリが起動します。それと並行して `tests` ディレクトリの中にあってファイル名の末尾が `.e2e.ts` であるテストが選択されて実行されます。全部のテストが終了するのを待って、webアプリが停止されます。
...
`bun:test` からimportされてた `describe` と `test` を使っていることに注意してほしい。[Playwright API](https://playwright.dev/docs/writing-tests#using-test-hooks) にも名前のよく似た `describe` と `test` があるのだが全く別のものだ。

`import * as PW from '@playwright/test';` としてPlaywrightのAPIを `PW` という名前空間にまとめている。`PW.Page` はPlaywrightのPageクラスである。

            const span: PW.Locator = page.getByText('GETリクエスト!');
            await PW.expect(span).toBeVisible();

ここで `PW.expect` という名前で呼び出しているのはPlaywrightの `expect` である。`` from bun:test`によってimportされた `expect `` ではない。

Playwrightの `page` オブジェクトと `locator` オブジェクトを使ってさまざまのactionを実行したり、DOM要素の状態を調べたりEventを捕捉したりすることができる。詳細な解説をネット上でたくさん見つけることができる。たとえばPlaywrightの公式ドキュメント [Writing tests](https://playwright.dev/docs/writing-tests) を参照のこと。

### ヘルパクラス `BrowserDriverChromium.ts` と `browser-helpers.ts`
...
\`BrowserDriverChromium\`オブジェクトはChromiumブラウザを立ち上げたり、URLにnavigateしたり、ブラウザをクローズするといった操作を実装している。テストコードが簡便にブラウザを操作できるようにしている。
...
\`browser-helpers\`の関数は 各種のパラメータやオブションに具体的な値を指定しつつPlaywright API を直に呼び出す。たとえばChromiumブラウザのウインドウをheadless（不可視）にするかどうかの指定や、ウインドウの縦横のサイズを指定することができる。詳細については Playwright APIの [Browser](https://playwright.dev/docs/api/class-browser) クラスと [BrowserType](https://playwright.dev/docs/api/class-browserType) クラスのAPI Referenceドキュメントを参照していただきたい。

## 6. まとめ

htmx構文を使ったwebアプリケーションをbunとHonoの上で構築しPlaywrightのライブラリを使ってE2Eテストすることができた。E2Eテストを実行するには `bun` に組み込まれたテストランナーを使った。細い疑問点をひとつひとつ解決するのはけっこう難しいかったが、その分学びも多かった。上記ではhtmx本のSection3のhtmxサンプルとテストだけを紹介したが、わたしは同じやり方でhtmx本のほぼ全部を写経し学ぶことができた。

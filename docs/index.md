...
なおhtmx本の CHAPTER01 から CHAPTER06 までの内容に対応するコードを `packages/my-app` というサブディレクトリに格納した。それに加えて CHAPTER07 のToDoアプリのコードを `packages/todo-app` というサブディレクトリに格納した。つまりひとつのGitレポジトリの中に複数のパッケージを収めた。いわゆる「monorepo」である。モノレポの作り方も説明する。
...
JSXを使うためには `packages/todo-app/tsconfig.json` に設定を加える必要がある。たとえばこんなふうに。
...
`packages/todo-app/package.json` に `"scripts": "bun run -hot ./src/todo.tsx"` を追加した。
...
`packages` ディレクトリの下に `my-app` パッケージと `todo-app` パッケージがある。それぞれのWebアプリはどちらもHonoを基盤としてJSXとhtmxを利用したアプリであって独立している。`` todo-app`のWebアプリのコードが`my-app`のコードに依存したりはしていない。これだけなら「モノレポ」の構成を作る必要はない。ところが詳しくは後で述べるが `todo-app `` のテストコードが `my-app` の中にある `BrowserDriverChromium` class を参照して使いたい。だから `todo-app` が `my-app` を参照できるようにbunの設定を作る必要があります。つまり「monorepo」を作ります。
...
これがあるので `packages` ディレクトリの下にある `my-app` ディレクトリと `todo-app` ディレクトリがそれぞれ [workspace](https://bun.com/docs/pm/workspaces) として認められることになります。

#### パッケージの name に注意

- 参考情報: [npm Docs / package.json](https://docs.npmjs.com/cli/v11/configuring-npm/package-json#name)

このプロジェクトには３つの `package.json` ファイルがありますが、その冒頭にある `name` プロパティの値をどうするかも重要です。

npm Docの name に関する説明はこう述べています。

> If you plan to publish your package, the most important things in your package.json are the name and version fields as they will be required. The name and version together form an identifier that is assumed to be completely unique. Changes to the package should come along with changes to the version. If you don’t plan to publish your package, the name and version fields are optional.

わたしはわたしのGitレポジトリ [htmx-and-playwright-tests-in-typescript](https://github.com/kazurayam/htmx-and-playwright-tests-in-typescript) の成果物をnpmにパブリッシュしようとは企んでいません。だからnmpレポジトリの中でuniqueな名前を与えなければならぬわけではありません。ただし `todo-app` が `my-app` の成果物を参照したいと企んでいます。それを可能にするには `todo-app` と `my-app` のパッケージ名を整った名前にしておくことが必須になります。だから仮にもnmpにパブリッシュできるようなユークな名前を付けるというのが実技として好ましい。
...
`packages/todo-app/packages.json` の name をこう書きました。
    ...
    {
        "name": "@kazurayam/htmx-and-playwright-tests-in-typescript-my-todo",

つまりルートのpackageのnameの形式を `@kazurayam/repositoryName` とし、モノレポを構成するパッケージのnameを `@kazurayam/repositoryName-パッケージ名` という形式にしました。 `kazurayam` とはわたし個人のGitHubアカウントのNameです。これはおそらくGlobalにユニークな名前空間として使える。packageのnameをこの形にするのはわたし個人のやり方です。参考まで。

REPOS直下の `package.json` で `"workspaces":["packages/*"]` を宣言し、`my-app` と `todo-app` の `package.json` のnameを適切に指定したら、REPOSにcdして下記のコマンドを実行します。

    $ cd $REPOS
    $ bun install

するとbunが設定ファイルを読み込んで内部の管理情報を更新します。

### 6.3 ToDoアプリのためにPlaywrightのテストを書いた

`packages/todo-app/tests/todo.e2e.ts` を実装した。
...
`todo.e2e.ts` はパッケージ '@kazurayam/htmx-and-playwright-tests-in-typescript-my-app' が持っていてexportしているはずの `BrowserDriverChromium` をimportして使いたい、と書いてある。

### 6.4 BrowserDriverChromiumが参照できない

ところがわたしが `todo.e2e.ts` を書いた時、VSCodeのエディタは下記のように下波線を表示して警告しました。
...
このエラーを回避するために `packages/todo-app/packages.json` を訂正した。
...
`devDependencies` プロパティに `"@kazurayam/htmx-and-playwright-tests-in-typescript-my-app": "workspace:*"` を追記した。この変更をbunに伝える必要がある。ルートディレクトリに戻って再度

    $ bun install

を実行した。

もう一度VSCodeのエディタに戻ってみた。まだ下波線が表示されていた。ただし警告メッセージが別の物になっていた。
...
`packages/my-app/package.json` ファイルの冒頭にこう書いてある.
...
`module` プロパティが `packages/my-app/index.ts` ファイルを指している。もう一度AIに聞いてみた。`module` プロパティって何？

> The **module property** in package.json is used to specify the entry point for ECMAScript modules (ESM) in a Node.js project, allowing the use of the import/export syntax. This property helps Node.js recognize which files to load as ESM, enhancing compatibility with modern JavaScript module systems.

なるほど。ということは `packages/my-app/index.ts` が `BrowserDriverChromium` を明示的にexportすればいい、のだろう。
...
修正を反映させるため、ルートディレクトリに戻って `bun install` コマンドを実行した。VSCodeのエディタで `todo.e2e.ts` を確認した。下波線が消えた。

### 6.5 todo-appでテストを実行

## 7. まとめ

htmx構文を使ったwebアプリケーションをbunとHonoの上で構築しPlaywrightのライブラリを使ってE2Eテストすることができた。E2Eテストを実行するのにbunに組み込まれたテストランナーを使った。Playwrightに関するドキュメントの多くは `npx playwright XXXX` というコマンドを使えと書いているが、あえてその方法をとらなかった。htmxを使ったwebアプリをPlaywrightでテストするという事例の紹介はネット上に見当たらない（と思う）。誰かの役に立てばいいなと思っています。

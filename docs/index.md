...
このコードは [`@hono/node-server`](https://github.com/honojs/node-server) を必要とする。だから下記のコマンドでプロジェクトに追加します。
...
ちなみに `@hono/node-server` はNode.jsのバージョン20以降を[必要とします](https://www.npmjs.com/package/@hono/node-server)。わたしのMacにはNode.js v24がインストール済みなので大丈夫。

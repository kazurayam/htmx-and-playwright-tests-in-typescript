...
- サーバーはTypeScript言語で書き、Bunの上で動かす。Bunがサーバー・サイドのWebSocket APIを提供しているのでこれを利用する。broadcastサービスを実現するのにBunの link:https://bun.com/docs/guides/websocket/pubsub[publish-subscribeパターンの実装] を活用する
...
- Chatアプリの３通りの手法で実装する。素朴な実装手法からより高度な手法へと段階的にコードを書き変えていく。
  ...
  2. 二番目のChatアプリでは link:https://htmx.org/extensions/ws/[HtmxのWebSocket Exetension] を導入する。カスタムなJavaScriptを不要にする。
...
[source,typescript]
----
include::../packages/chat-app/src/htmx-ws/index.ts[lines=30..38]
----
...
{
  "scripts": {
include::../packages/chat-app/package.json[lines=14..20]
----

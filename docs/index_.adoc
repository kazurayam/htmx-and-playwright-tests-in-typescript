...
わたしはGitレポジトリ link:{REPOS_URL}[htmx-and-playwright-tests-in-typescript] の成果物をnpmにパブリッシュするつもりはない。だからnmpレポジトリの中でuniqueな名前を与えなければならぬ義理はない。しかし `todo-app` が `my-app` の成果物を参照したいと企んでい。それを可能にするには `todo-app` と `my-app` のパッケージ名を整った名前にしておくことが必須になります。だからnmpにパブリッシュできるようなユークな名前を付けることを常日頃から心がけておく方が得策だ。

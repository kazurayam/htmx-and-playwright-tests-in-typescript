...
htmx構文を使ったwebアプリケーションをbunとHonoの上で構築しPlaywrightのライブラリを使ってE2Eテストすることができた。E2Eテストを実行するのにbunに組み込まれたテストランナーを使った。Playwrightに関するドキュメントの多くは `npx playwright XXXX` というコマンドを使えと書いているが、あえてその方法をとらなかった。PlaywrightのAPIを利用してブラウザのプロセスを起動・終了するためのクラス（`BrowserDriverChromium`）と関数を独自に実装した。htmxを使ったwebアプリをbunの上でPlaywrightでテストするという事例を紹介した記事はネット上にほかに見当たらないと思う。誰かの役に立てばいいなと思っています。

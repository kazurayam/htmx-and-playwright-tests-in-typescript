...
<figure>
<img src="{PAGES}/images/007_section3_before_action.png" alt="007 section3 before action" />
</figure>
...
htmx本にはhtmxのたくさんの機能を紹介するサンプルコードが紹介されています。htmx本を読みながら、上記で `src/section3.jsx` を作ったのと同じようにして \`section5.tsx\`を追加し、\`section6.tsx\`を追加していくことができます。
...
console.log()によるメッセージが無くなってコンソールの表示がスッキリしました。いっぽうで `$PROJECT/out/my-app.log` ファイルが作られ、次のようなログが出力されました。
...
テストコードのみならずwebアプリ本体もロギングフレームワークを利用して、詳細なログ情報をファイルに出力することができます。

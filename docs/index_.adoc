...
- htmx本はテストをどう書くかについて言及していない。しかしわたしはhtmx本のサンプルコードを写したらテストを書いて、逐一動作を確認するというtest-firstな手順でやりたい。わたしは link:https://playwright.dev/docs/intro[Playwright] でテストを書こう。

=== わたしが取り組むべき課題

Playwrightの公式ドキュメント link:https://playwright.dev/docs/intro[Getting started/Installation] を見れば明らかだが、Playwrightのドキュメントは link:https://nodejs.org/[Node.js] 上で実行することを前提している。bunの上でPlaywrightを動かす例がネット上にほとんど見当たらない。だからわたしは自力で答えを見つけなければならない。
...
image::{PAGES_URL}/images/main.ts.png[]
